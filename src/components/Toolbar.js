...
import { FiDownload, FiUpload, FiArrowLeft, FiArrowRight, FiLayers } from 'react-icons/fi';
...
const Toolbar = () => {
  ...
  const handleDownload = (code, language) => {
    const blob = new Blob([code], { type: language === 'javascript' ? 'application/javascript' : language === 'jsx' ? 'text/jsx' : 'text/html' });
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = `project.${language === 'javascript' ? 'js' : language === 'jsx' ? 'jsx' : 'html'}`;
